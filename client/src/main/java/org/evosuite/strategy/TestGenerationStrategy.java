...
public abstract class TestGenerationStrategy {
	...
	protected List<TestSuiteNoveltyFunction> getNoveltyFunctions() {
	    List<TestSuiteNoveltyFunction> nfs = new ArrayList<TestSuiteNoveltyFunction>();
	    for (int i = 0; i < Properties.CRITERION.length; i++) {
	...
	public static List<TestNoveltyFactory<? extends TestNoveltyFunction>> getNoveltyFactories() {
	    List<TestNoveltyFactory<? extends TestNoveltyFunction>> goalsFactory = new ArrayList<TestNoveltyFactory<? extends TestNoveltyFunction>>();
	    for (int i = 0; i < Properties.CRITERION.length; i++) {
	        goalsFactory.add(NoveltyFunctions.getNoveltyFactory(Properties.CRITERION[i]));
	    }
