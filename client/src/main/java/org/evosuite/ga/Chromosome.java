...
public abstract class Chromosome implements Comparable<Chromosome>, Serializable,
	...
	/** Last recorded of novelty metric value */
	private LinkedHashMap<NoveltyFunction<?>, Double> noveltyValues = new LinkedHashMap<NoveltyFunction<?>, Double>();
	...
	/** Previous novelty value, to see if there was an improvement */
	private LinkedHashMap <NoveltyFunction<?>, Double> previousNoveltyValues = new LinkedHashMap<NoveltyFunction<?>, Double>();
	
	/* Archive the selected novelty values */
	//private Archive<Chromosome> archieveforNovelty = new Archive<Chromosome>() {
		/*
		@Override
		public Chromosome createMergedSolution(Chromosome solution) {
	...
	*/
	...
	private LinkedHashMap<NoveltyFunction<?>, Double> coverageValuesNovelty = new LinkedHashMap<NoveltyFunction<?>, Double>();
	
	private LinkedHashMap<FitnessFunction<?>, Integer> numsNotCoveredGoals = new LinkedHashMap<FitnessFunction<?>, Integer>();

	private LinkedHashMap<NoveltyFunction<?>, Integer> numsNotCoveredGoalsNovelty = new LinkedHashMap<NoveltyFunction<?>, Integer>();
	
	private LinkedHashMap<FitnessFunction<?>, Integer> numsCoveredGoals = new LinkedHashMap<FitnessFunction<?>, Integer>();
	
	private LinkedHashMap<NoveltyFunction<?>, Integer> numsCoveredGoalsNovelty = new LinkedHashMap<NoveltyFunction<?>, Integer>();

	
	/* Implementation Start For Novelty Search
	 * Author : Sefa Akca
	 * 
	 */
	/**
	 *  It returns the current novelty value
	 * @return double
	 */
	public double getNovelty(){
		
		if(noveltyValues.size() > 1){
			double sumNovelty = 0.0;
			for(NoveltyFunction<?> noveltyFunction : noveltyValues.keySet())
			{
				sumNovelty += noveltyValues.get(noveltyFunction);
			}
			return sumNovelty;
		}
		else
			return noveltyValues.isEmpty() ? 0.0 : noveltyValues.get(noveltyValues.keySet().iterator().next());
		
	}
	
	public Map<NoveltyFunction<?>, Double> getNoveltyValues()
	{
		return this.noveltyValues;
	}
	
	public Map<NoveltyFunction<?>, Double> getPreviousNoveltyValues()
	{
		return this.previousNoveltyValues;
	}
	
	public void setNoveltyValues(Map<NoveltyFunction<?>, Double> fits)
	{
		this.noveltyValues.clear();
		this.noveltyValues.putAll(fits);
	}
	
	public void setPreviousNoveltyValues(Map<NoveltyFunction<?>, Double> lastfits)
	{
		this.previousNoveltyValues.clear();
		this.previousNoveltyValues.putAll(lastfits);
	}
	...
	public void addNovelty(NoveltyFunction<?> nf, double noveltyValue)
	{
		this.addNovelty(nf, noveltyValue, 0.0, 0);
	}
	
	public void addNovelty(NoveltyFunction<?> nf, double noveltyValue, double coverage)
	{
	...
	public void setNovelty(NoveltyFunction<?> nf, double value) throws IllegalArgumentException
	{
		if((Double.compare(value, Double.NaN) == 0) || Double.isInfinite(value))
		{
			throw new IllegalArgumentException("Invalid value of novelty: " + value + ", Novelty: "+ nf.getClass().getName());
		}
		if(!noveltyValues.containsKey(nf))
		{
			previousNoveltyValues.put(nf, value);
			noveltyValues.put(nf, value);
		}
		else
		{
			previousNoveltyValues.put(nf, noveltyValues.get(nf));
			noveltyValues.put(nf, value);
		}
	}
...
//				 || ( value < 0 ) || ( ff == null )) 
	...
	public double getCoverageNovelty(){
		double sum = 0;
		for(NoveltyFunction<?> noveltyFunction : coverageValuesNovelty.keySet()){
			sum += coverageValuesNovelty.get(noveltyFunction);
		}
		double cov = coverageValuesNovelty.isEmpty() ? 0.0 : sum / coverageValuesNovelty.size();
		assert (cov >= 0.0 && cov <= 1.0) : "Incorrect coverage value " + cov + ". Expected value between 0 and 1";
        return cov;
	}
	...
	public void setNumOfNotCoveredGoals(NoveltyFunction<?> nf, int numCoveredGoals) {
		this.numsNotCoveredGoalsNovelty.put(nf, numCoveredGoals);
	}
	...
	public Map<NoveltyFunction<?>, Double> getCoverageValuesNovelty() {
		return this.coverageValuesNovelty;
	}
	...
	public void setCoverageNovelty(NoveltyFunction<?> nf, double coverage) {
		this.coverageValuesNovelty.put(nf, coverage);
	}
	...
	public int getNumOfCoveredGoalsNovelty(NoveltyFunction<?> nf) {
		return numsCoveredGoalsNovelty.containsKey(nf) ? numsCoveredGoals.get(nf) : 0;
	}
	...
	public int getNumOfNotCoveredGoalsNovelty(NoveltyFunction<?> nf) {
		return numsNotCoveredGoalsNovelty.containsKey(nf) ? numsNotCoveredGoalsNovelty.get(nf) : 0;
	}
	...
	public void setNumOfCoveredGoals(NoveltyFunction<?> nf, int numCoveredGoals) {
		this.numsCoveredGoalsNovelty.put(nf, numCoveredGoals);
	}
