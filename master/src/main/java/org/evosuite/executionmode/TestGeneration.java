...
public class TestGeneration {
	...
	public static Option[] getOptions() {
		return new Option[] { new Option("generateSuite", "use whole suite generation. This is the default behavior"),
	...
	private static List<List<TestGenerationResult>> generateTests(Properties.Strategy strategy, String target,
		...
		if (!findTargetClass(target)) {
			return Arrays.asList(Arrays.asList(new TestGenerationResult[] {
					TestGenerationResultBuilder.buildErrorResult("Could not find target class") }));
		}
		 ...
		 * First 2 does not work for master, as logback is read before
		...
		if (Properties.DEBUG) {
			// enabling debugging mode to e.g. connect the eclipse remote
			// debugger to the given port
			cmdLine.add("-Ddebug=true");
			cmdLine.add("-Xdebug");
			cmdLine.add("-Xrunjdwp:transport=dt_socket,server=y,suspend=y,address=" + Properties.PORT);
			LoggingUtils.getEvoLogger()
					.info("* Waiting for remote debugger to connect on port " + Properties.PORT + "..."); // TODO
																											// find
																											// the
																											// right
			// place for this
		...
		if (Properties.JMC) {
			// FIXME: does not seem to work, at least on Mac. Looks like some
			// RMI conflict
		...
		/*
		 * TODO: here we start the client with several properties that are set
		 * through -D. These properties are not visible to the master process
		 * (ie this process), when we access the Properties file. At the moment,
		 * we only need few parameters, so we can hack them
		 */
		 ...
		 * 
		 * The use of "assertions" in the client is pretty tricky, as those
		 * properties need to be transformed into JVM options before starting
		 * the client. Furthermore, the properties in the property file might be
		...
		/*
		 * the assertions might not be defined in the command line, but they
		 * might be in the property file, or just use default values. NOTE: if
		 * those are defined in the command line, then they overwrite whatever
		 * we had in the conf file
		 */
		...
		/*
		 * We add them in first position, after the java command To avoid
		 * confusion, we only add them if they are enabled. NOTE: this might
		 * have side effects "if" in the future we have something like a generic
		 * "-ea"
		 */
		...
		if (!Properties.CLIENT_ON_THREAD) {
			/*
			 * We want to completely mute the SUT. So, we block all outputs from
			 * client, and use a remote logging
			 */
		...
		if (handler.startProcess(newArgs)) {
			...
			if (clients == null) {
				...
				/*
				 * The clients have started, and connected back to Master. So
				 * now we just need to tell them to start a search
				 */
		...
		/*
		 * FIXME: it is unclear what is the relation between
		 * TestGenerationResult and writeStatistics()
		 */
		...
		if (hasFailed) {
			...
			// note: cannot throw exception because would require refactoring of
			// many SystemTests
